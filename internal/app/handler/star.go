...
func (h *Handler) GetStars(ctx *gin.Context) {
	...
	stars, err := h.Repository.GetStars(payload.Role, payload.UserID, status, startDateStr != "", endDateStr != "", startDate, endDate)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, err.Error())
		return
	}
...
// @Tags Star
// @Param id path string true "Star ID"
// @Param calc_values_{scope_id} formData string false "Calculation values"
// @Success 200 {object} map[string]string
// @Failure 400 {object} map[string]string
// @Failure 500 {object} map[string]string
