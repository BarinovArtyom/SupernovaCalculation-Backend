...
func (r *Repository) UpdateCalculationResult(result async.CalculationResult) error {
	// Парсим IDs как числа
	starID, err := strconv.Atoi(result.StarID)
	if err != nil {
		return fmt.Errorf("invalid star_id format: %s", result.StarID)
	}

	scopeID, err := strconv.Atoi(result.ScopeID)
	if err != nil {
		return fmt.Errorf("invalid scope_id format: %s", result.ScopeID)
	}

	r.logger.Infof("Updating calculation: star_id=%d, scope_id=%d", starID, scopeID)

	// Обновляем запись в таблице calcs
	...
	if err != nil {
		return fmt.Errorf("cannot save calculation result: %w", err)
	}

	r.logger.Infof("Calculation updated successfully")
	return nil
}
