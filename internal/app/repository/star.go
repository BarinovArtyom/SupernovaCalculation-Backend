...
func (r *Repository) GetStars(status string, hasStartDate, hasEndDate bool, startDate, endDate time.Time) (*[]ds.Star, error) {
	var stars []ds.Star

	query := r.db.Model(&ds.Star{}).
		Where("stars.status NOT IN (?, ?)", "deleted", "active") // Исключаем удаленные и черновики
...
func (r *Repository) FormStar(starID string, creatorID string, calcValues map[string]string) error {
	var star ds.Star
	...
	intCreator, err := strconv.Atoi(creatorID)
	if err != nil {
		return err
	}

	if star.UserID != intCreator {
...
func (r *Repository) SetStarStatus(starID string, status string) error {
	var star ds.Star
	if err := r.db.First(&star, starID).Error; err != nil {
		return err
	}

	modID, err := dsn.GetCurrentUserID()
	if err != nil {
		return err
	}
	intModID, err := strconv.Atoi(modID)
	if err != nil {
		return err
	}
